...
from phrase_stitching.RN_analysis import analyze_entire_phrase, get_vertical_pairs, InvalidAnalysisException, \
    check_illegal_harmonics_on_integer_beats, check_bad_counterpoint, check_bad_mode_mixture
...
def prepare_phrase(score, analysis, show=True):
    """Extend last note to fill measure, write inner voices, and optionally show the phrase."""
    score, analysis = extend_last_note_to_fill_measure(score, analysis)
...
def combine_two_scores(score1, score2):
    ...
    for part1, part2 in zip(score1.parts, score2.parts):
        ...
        if first_tempo:
            combined_part.insert(0, copy.deepcopy(first_tempo))
        if time_sig:
            combined_part.insert(0, copy.deepcopy(time_sig))
...
def stitch_i_III_iv_i(score_analysis_starts, score_analysis_ends, score_analysis, show_phrases=True):
    beginning = None
    middle = None
    middle2 = None
    end = None
    ...
    beginning, beginning_analysis = prepare_phrase(beginning, score_analysis[beginning], show=show_phrases)
    middle, middle_analysis = prepare_phrase(middle, score_analysis[middle], show=show_phrases)
    middle2, middle2_analysis = prepare_phrase(middle2, score_analysis[middle2], show=show_phrases)
    end, end_analysis = prepare_phrase(end, score_analysis[end], show=show_phrases)

    middle = transpose_score(middle, 3)
    middle2 = transpose_score(middle2, 5)

    final = combine_two_scores(beginning, middle)
    final = combine_two_scores(final, middle2)
    final = combine_two_scores(final, end)
    return final


def stitch_i_III_V_i(score_analysis_starts, score_analysis_ends, score_analysis, show_phrases=True):
    beginning = None
    middle = None
    middle2 = None
    end = None
...
def stitch_I_V_I(score_analysis_starts, score_analysis_ends, score_analysis, show_phrases=True):
    beginning = None
    middle = None
    middle2 = None
    end = None
...
def stitch_I_IV_V_I(score_analysis_starts, score_analysis_ends, score_analysis, show_phrases=True):
    beginning = None
    middle = None
    middle2 = None
    end = None
...
def stitch_i_VI_iv_i(score_analysis_starts, score_analysis_ends, score_analysis, show_phrases=True):
    beginning = None
    middle = None
    middle2 = None
    end = None
